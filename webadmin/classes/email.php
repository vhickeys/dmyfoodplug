...
function invoicePaidMail($sendingMail, $recepientMail, $recepient, $tracking_no, $date, $subtotal, $discount, $shipping, $grand_total, $address, $city, $country, $phone, $office_phone, $office_email, $office_address, $order_items_html, $shipping_location)
{
    //Create an instance; passing `true` enables exceptions
    $mail = new PHPMailer(true);
    ...
    try {
        ...
        //Content
        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = "Dmy Foodplug Invoice - Payment Successfully Received!";

        // Load the HTML template from a file
        $placeholders = array('[Name]', '[Status]', '[Tracking No]', '[Date]', '[Subtotal]', '[Discount]', '[Shipping]', '[Grand Total]', '[Address]', '[City]', '[Country]', '[Phone]', '[Email]', '[Office Phone]', '[Office Email]', '[Office Address]', '[OrderItems]', '[Shipping Location]');
        $replacements = array($recepient, "Payment Successfully Received", $tracking_no, $date, $subtotal, $discount, $shipping, $grand_total, $address, $city, $country, $phone, $recepientMail, $office_phone, $office_email, $office_address, $order_items_html, $shipping_location);

        $message = file_get_contents('email-templates/invoice-paid.html'); // Replace with the path to your HTML template file
        $message = str_replace($placeholders, $replacements, $message); // Replace [Username] with the actual username
        $mail->Body = $message;
...
function invoicePaidNotice($sendingMail, $recepientMail, $recepient, $tracking_no, $date, $subtotal, $discount, $shipping, $grand_total, $address, $city, $country, $phone, $office_phone, $office_email, $office_address, $order_items_html, $shipping_location)
{
    //Create an instance; passing `true` enables exceptions
    $mail = new PHPMailer(true);
    ...
    try {
        ...
        $message = file_get_contents('email-templates/invoice-paid-notice.html'); // Replace with the path to your HTML template file
        $message = str_replace($placeholders, $replacements, $message); // Replace [Username] with the actual username
        $mail->Body = $message;
